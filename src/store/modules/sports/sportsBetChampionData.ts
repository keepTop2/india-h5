...
import { SportPushApi, WebToPushApi } from "/@/views/venueHome/sports/enum/sportEventSourceEnum";
import { WorkerTransfer } from "/@/models/webWorkerModel";
import pubsub from "/@/pubSub/pubSub";
import { championInfo } from "/@/views/venueHome/sports/utils/interface";
...
export const useSportsBetChampionStore = defineStore("sportsBetChampion", {
	state: () => {
		return {
			...
			championBetData: [] as championInfo[], // 冠军购物车数据
			championBetObj: {} as championInfo, // 冠军购物车数据
			bettingStatus: 0,
		};
	},
	getters: {
	...
	actions: {
		...
		championOpenSse() {
			const leagueIds: any = [];
			for (const key in this.championBetObj) {
				if (Object.prototype.hasOwnProperty.call(this.championBetObj, key)) {
					const leagueId = this.championBetObj[key].leagueId;
		...
		clearChampionShopCart() {
			this.championBetData = [] as championInfo[];
			this.championBetObj = {} as championInfo;
			this.championCloseSse();
			this.closeChampionShopCart();
		},
	},
