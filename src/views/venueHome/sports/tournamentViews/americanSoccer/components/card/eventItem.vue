<template>
	...
	<div class="content">
		...
		<div class="content-header">
			...
			<!-- 盘口表头  -->
			<div class="right">
				<div class="title">{{ SportsCommonFn.betTypeMap[3][20] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[3][1] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[3][3] }}</div>
			</div>
		</div>
		<!-- 内容区域 主体 -->
		<div class="content-info">
			<div class="content-left" @click="showDetail(event.eventId, event.leagueId)">
				<div class="team-tournament-info" :class="SportsCommonFn.isStartMatch(event.globalShowTime) ? 'mt_18' : 'mt_58'">
					...
					<!-- 红牌黄牌数量 -->
					<div class="foul-info">
						<span v-if="event.soccerInfo?.homeRedCard > 0" class="red">{{ event.soccerInfo?.homeRedCard }}</span>
						<span v-if="event.soccerInfo?.homeYellowCard > 0" class="yellow">{{ event.soccerInfo?.homeYellowCard }}</span>
					</div>
				</div>
				<div class="team-tournament-info" :class="SportsCommonFn.isStartMatch(event.globalShowTime) ? 'mt_32' : 'mt_52 pb_23'">
					...
					<!-- 红牌黄牌数量 -->
					<div class="foul-info">
						<span v-if="event.soccerInfo?.awayRedCard > 0" class="red">{{ event.soccerInfo?.awayRedCard }}</span>
						<span v-if="event.soccerInfo?.homeRedCard > 0" class="yellow">{{ event.soccerInfo?.homeRedCard }}</span>
					</div>
				</div>
				<!-- 主队客队比分 -->
