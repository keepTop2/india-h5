		...
		<div class="item" v-for="(item, index) in market.selections" :key="index">
			<div class="data_item" v-if="item">
				<div v-if="market.marketStatus === 'running'" :class="{ isBright: isBright(item) }" @click="onSetSportsEventData(item)">
					<div class="label">
						<span v-if="market.betType == 5 || market.betType == 15">{{ item?.keyName }}</span>
						<span v-if="market.betType == 1 || market.betType == 7">{{ SportsCommon.formatPoint({ betType: market.betType, point: item?.point, key: item?.key }) }}</span>
						<span v-if="market.betType == 3 || market.betType == 8">
							{{ item?.keyName }}
							{{ item?.point }}
						</span>
					</div>
