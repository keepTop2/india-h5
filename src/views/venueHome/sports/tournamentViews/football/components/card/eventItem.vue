<template>
	<!-- 赛事内容区域 -->
	<div class="content">
		<!-- 内容区域 头部 -->
		<div class="content-header">
			<!-- 上下半场及时间 -->
			<div class="left">
				<div class="icon">
					<!-- 关注 -->
					<SvgIcon class="icon" v-if="isAttention" iconName="/venueHome/sports/svg/collection2" @click="attentionEvent(true)" />
					<!-- 取消关注 -->
					<SvgIcon class="icon" v-else iconName="/venueHome/sports/svg/collection" @click="attentionEvent(false)" />
				</div>

				<div class="label" :class="{ 'color_Theme-P': event.isLive, color_TB: !event.isLive }">
					<span class="mr_6">{{ SportsCommonFn.getEventsTitle(event) }}</span>
					<span v-if="(event.gameInfo.livePeriod == 2 || event.gameInfo.livePeriod == 1) && !event.gameInfo.delayLive && !event.gameInfo.isHt">{{ formattedGameTime }}</span>
				</div>
			</div>
			<!-- 盘口表头  -->
			<div class="right" v-if="!isFULL">
				<div class="title">{{ SportsCommonFn.betTypeMap[1][5] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[1][1] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[1][3] }}</div>
			</div>
			<div class="right" v-else>
				<div class="title">{{ SportsCommonFn.betTypeMap[1][15] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[1][7] }}</div>
				<div class="title">{{ SportsCommonFn.betTypeMap[1][8] }}</div>
			</div>
		</div>
		<!-- 内容区域 主体 -->
		<div class="content-info">
			<div class="content-left" @click="showDetail(event.eventId, event.leagueId)">
				<div class="team-tournament-info" :class="SportsCommonFn.isStartMatch(event.globalShowTime) ? 'mt_18' : 'mt_58'">
					<!-- 队伍icon -->
					<div class="icon">
						<VantLazyImg :src="event.teamInfo?.homeIconUrl" />
					</div>
					<!-- 队伍名称 -->
					<div class="name">{{ event.teamInfo.homeName }}</div>
					<!-- 红牌黄牌数量 -->
					<div class="foul-info" v-if="event.soccerInfo?.homeRedCard == 0 && event.soccerInfo?.homeYellowCard == 0">
						<span class="red" :class="{ hide: event.soccerInfo?.homeRedCard === 0 }">{{ event.soccerInfo?.homeRedCard }}</span>
						<span class="yellow" :class="{ hide: event.soccerInfo?.homeYellowCard === 0 }">{{ event.soccerInfo?.homeYellowCard }}</span>
					</div>
				</div>
				<div class="team-tournament-info" :class="SportsCommonFn.isStartMatch(event.globalShowTime) ? 'mt_32' : 'mt_52 pb_23'">
					<!-- 队伍icon -->
					<div class="icon">
						<VantLazyImg :src="event.teamInfo?.awayIconUrl" />
					</div>
					<!-- 队伍名称 -->
					<div class="name">{{ event.teamInfo.awayName }}</div>
					<!-- 红牌黄牌数量 -->
					<div class="foul-info" v-if="event.soccerInfo?.awayRedCard == 0 && event.soccerInfo?.awayYellowCard == 0">
						<span class="red" :class="{ hide: event.soccerInfo?.awayRedCard === 0 }">{{ event.soccerInfo?.awayRedCard }}</span>
						<span class="yellow" :class="{ hide: event.soccerInfo?.awayYellowCard === 0 }">{{ event.soccerInfo?.awayYellowCard }}</span>
					</div>
				</div>
				<!-- 主队客队比分 -->
				<div class="score-info" v-if="SportsCommonFn.isStartMatch(event.globalShowTime)">
					<span>{{ event.gameInfo?.liveHomeScore }}</span>
					<i class="icon"></i>
					<span>{{ event.gameInfo?.liveAwayScore }}</span>
				</div>
				<!-- 角球等信息 -->
				<div class="others">
					<!-- 上半场比分 下半场不显示 -->
					<div v-if="event.gameInfo?.livePeriod == 1" class="other2">
						<span>HT</span>
						<span>{{ `${event.gameInfo?.liveHomeScore}-${event.gameInfo?.liveAwayScore}` }}</span>
					</div>
					<div v-if="event.streamingOption != 0 && event.channelCode" class="other3">
						<SvgIcon iconName="/venueHome/sports/svg/sport_live" />
					</div>
					<!-- <div class="other4">
						<SvgIcon iconName="/venueHome/sports/svg/sport_match_status" size="4.266667" />
					</div> -->
					<div class="other5">{{ event.marketCount }}</div>
				</div>
			</div>
			<!-- 全场 半场 swiper  -->
			<div class="content-right">
				<div class="arrow" :class="{ arrow_reversal: isFULL }">
					<img :src="left_arrow" alt="" />
				</div>
				<Swiper class="swiper" :spaceBetween="4" :pagination="true" :modules="modules" @slideChange="firstSlideChange($event)">
					<swiper-slide>
						<div class="content">
							<marketItem v-for="market in filterMarkets(event.markets, [5, 1, 3])" :key="`${event.eventId}-${market.betType}`" :event="event" :market="market" />
						</div>
					</swiper-slide>
					<swiper-slide>
						<div class="content">
							<marketItem v-for="market in filterMarkets(event.markets, [15, 7, 8])" :key="`${event.eventId}-${market.betType}`" :event="event" :market="market" />
						</div>
					</swiper-slide>
				</Swiper>
			</div>
		</div>
	</div>
</template>

<script setup lang="ts">
import "swiper/css";
import "swiper/css/pagination";
import "swiper/css/navigation";
import marketItem from "./marketItem.vue";
import left_arrow from "/@/assets/zh-CN/default/venueHome/sports/left_arrow.gif";
import { useRouter } from "vue-router";
import { Swiper, SwiperSlide } from "swiper/vue";
import { Pagination, Navigation, Autoplay } from "swiper/modules";
import { Market, SportsRootObject } from "/@/views/venueHome/sports/utils/interface";
import { useSportsBetEventStore } from "/@/store/modules/sports/sportsBetData";
import sportsApi from "/@/api/venueHome/sports";
import pubsub from "/@/pubSub/pubSub";
const sportsBetData = useSportsBetEventStore();
// import { convertUtcToUtc5AndFormatMD } from "/@/webWorker/module/utils/formattingChildrenViewData";
import SportsCommonFn from "/@/views/venueHome/sports/utils/common";
import { i18n } from "/@/i18n/index";

const props = defineProps<{
	event: SportsRootObject;
	sportType: String;
}>();

const $: any = i18n.global;
const router = useRouter();
const modules = ref([Autoplay, Pagination, Navigation]); //swiper配置项
const isFULL = ref(0); // swiper标记

const isAttention = computed(() => {
	return sportsBetData.attentionEventIdList.includes(props.event.eventId);
});

// 点击关注按钮
const attentionEvent = async (isActive: boolean) => {
	if (isActive) {
		await sportsApi.unFollow({
			thirdId: [props.event.eventId],
		});
	} else {
		await sportsApi.saveFollow({
			thirdId: props.event.eventId,
			type: 2,
		});
	}
	pubsub.publish(pubsub.PubSubEvents.SportEvents.attentionChange.eventName, {});
};

// row字典 不同的betType显示几行
const DEFAULT_SELECTIONS_MAP: Record<number, number> = {
	[SportsCommonFn.BetTypeEnum.FULL_TIME_WIN]: 3,
	[SportsCommonFn.BetTypeEnum.HALF_TIME_WIN]: 3,
	[SportsCommonFn.BetTypeEnum.FULL_TIME_HANDICAP]: 2,
	[SportsCommonFn.BetTypeEnum.FULL_TIME_OVER_UNDER]: 2,
	[SportsCommonFn.BetTypeEnum.HALF_TIME_HANDICAP]: 2,
	[SportsCommonFn.BetTypeEnum.HALF_TIME_OVER_UNDER]: 2,
};

// 生成盘口信息
const filterMarkets = (markets: Market[], betTypes: number[]): Market[] => {
	const results: Market[] = [];
	betTypes.forEach((type) => {
		const marketItem = markets[type];
		if (marketItem) {
			results.push(marketItem);
		} else {
			const selectionsCount = DEFAULT_SELECTIONS_MAP[type] || 0;
			results.push({ betType: type, selections: Array(selectionsCount).fill(null) } as Market);
		}
	});
	return results;
};

// 滑动盘口信息
const firstSlideChange = (event) => {
	isFULL.value = event.activeIndex;
};

// 定义计算属性 格式化比赛开始时间
const formattedGameTime = computed(() => {
	const minutes = Math.floor(props.event.gameInfo.seconds / 60);
	const seconds = props.event.gameInfo.seconds % 60;
	return `${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`;
});

// 跳转详情
const showDetail = (eventId, leagueId) => {
	router.push(`/venueHome/sports/event/detail/${eventId}/${leagueId}/${props.event.sportType}`);
};
</script>

<style scoped lang="scss">
@import "/@/views/venueHome/sports/common.scss";
</style>
