	...
	<div class="bet_slip_cell_one">
		<div class="bet_slip_cell_input_one" @click="onBetNumber">
			<input
				v-model="stake"
				type="text"
				:placeholder="`限额 ${common.getInstance().formatFloat(sportsBetInfo.singleTicketInfo.minBet)} ~ ${common.getInstance().formatFloat(sportsBetInfo.singleTicketInfo.maxBet)}`"
				readonly
			/>
...
// 	(newMaxBet) => {
// 		if (newMaxBet) stake = computed(() => shopCartPubSub.getSingleTicketBetValue());
// 	}
// );

const onBetNumber = () => {
	shopCartPubSub.setBetNumberShow();
};
</script>
