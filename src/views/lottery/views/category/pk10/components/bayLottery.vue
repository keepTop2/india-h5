<template>
	<div class="content">
			...
			<div style="width: 100%; flex: 1">
				...
				>
					...
					<template v-if="gameplayItem.actived" #content>
						...
						>
							<template v-if="oddsListItem.type === 'selectBall' || oddsListItem.type === 'selectBallLine'" #right>
							...
							<template v-if="oddsListItem.actived && oddsListItem.type === 'selectBallLine'" #default>
								<div class="accordion-content-item-balls selectBallLine">
									<div class="gameplay gameplay-description">
										<!-- 使用 v-html 渲染描述内容，支持 HTML 标签如 <br> -->
		...
		<BetForm ref="betFormRef" @submit="handleSubmit" :value="currentGameplayItem" :actived="formActived" :currentOddsListItem="currentOddsListItem" :lotteryDetail="lotteryDetail">
			<template #betContent>
				<div v-if="!isSelectBall && currentOddsListItem.type !== 'selectBallLine'">
					<div class="title">{{ currentOddsListItem.optionName }}</div>
					<div class="desc">{{ currentOddsListItem.desc }}</div>
				</div>
				<div v-else>
