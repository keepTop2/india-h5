		...
		<BetForm ref="betFormRef" @submit="handleSubmit" :value="currentGameplayItem" :actived="formActived" :currentOddsListItem="currentOddsListItem" :lotteryDetail="lotteryDetail">
			<template #betContent>
				<div v-if="!isSelectBall">
					...
					<div class="desc">{{ currentOddsListItem.title }}</div>
				</div>
				<div v-else>
...
const { BetForm, Order, openBet, closeBet } = useBetForm();
...
console.log(currentGameplayItem, currentOddsListItem, "currentOddsListItem");

const isShowOrder = computed(() => {
	const flag = Object.keys(currentOddsListItem.value).length || Object.keys(currentGameplayItem.value).length;
	if (currentOddsListItem.value.type === SELECT_BALL) {
		return flag && balls.value.length;
	}
	return flag;
});
</script>
